...
int main(int argc, char* argv[]) {
	...
	while(1) {			
		...
		int tp=0;										//gives whether it has '&'or'&&'or'&&&'or none
		lines[0]=(char*)malloc(MAX_TOKEN_SIZE*sizeof(char));
		//splitting the command with '&'s
		...
		if(tp==0){										//if no '&' in the command line
			...
			else if(tokens[0][0]=='e' && tokens[0][1]=='x' && tokens[0][2]=='i' && tokens[0][3]=='t' && tokens[0][4]=='\0'){
				int killed=kill(0,SIGINT);
			}
