...
int numpp(void)
{
  ...
  for(int i = 0; i<NPDENTRIES/2; i++){ //remaining half is kernel based pages
    pde = &pgdir[i];
    if ( *pde & PTE_P){
      for(int j = 0; j<NPTENTRIES; j++){
        // Here, ith PDE has physical address of ith pagetable.
        // We need the virtual address. Hehe
        // Then we read the jth PTE in the ith pgtable to check if it is valid
        pgtable = (pte_t *)P2V(PTE_ADDR(*pde));
        if (pgtable[j] & PTE_P) ppcount++;
      }
    }
  }
  return ppcount;
...
{
  if(n <= 0) return 0;
  if(n % PGSIZE) return 0;

  struct proc* curproc = myproc();
  uint sz = curproc->sz;

  if(sz + (uint)n > KERNBASE){
    return 0;
  } 
  curproc->sz = sz + (uint)n; // increasing the virtual memory

  // No need for switchuvm(curproc)... as no change to pgtable or BTB
